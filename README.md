...
We'll run the tests in a container:

```bash
docker container run -it --rm -v $(pwd):/app/ php:8.2-cli bash
...
# Ensure we can talk to the test server
curl -i http://localhost:4020

composer test

# or run a specific test
./vendor/bin/phpunit --filter it_can_report_broken_urls_in_the_mail tests/MailBrokenUrlsTest.php

# looking for the laravel log file created by the tests?
